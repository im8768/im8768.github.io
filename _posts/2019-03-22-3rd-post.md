...
액션 리스트와 서비스 리스트는 drain_action_queue() 함수를 통해 실행함

```static void drain_action_queue(void)
{
   struct listnode *node;
   struct command *cmd;
   struct action *act; // 실행할 명령어들의 액션 리스트를 가지고 있음
   int ret;

   while ((act = action_remove_queue_head())) { // 액션 큐의 연결리스트 헤더 획득
      INFO(“processing action %p (%s)\n”, act, act -> name); 
      list_for_each(vode, &act -> commands) {
         cmd = node_to_item(node, struct command, clist); // 커맨드 구조체로 변환
         ret = cmd->func(cmd->nargs, cmd->args); 
// func 변수는 액션 리스트 명령어에 연결된 실행함수
      }
   }
}
```
...
* 서비스 리스트 내의 프로세스 실행
on boot 섹션의 class_start 명령어 service 섹션에 있는 모든 프로세스를 실행
class_start 명령어는 do_class_start() 실행 함수와 연결됨
